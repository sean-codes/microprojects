...
var myMention = new Mention({
   ...
   options: [{ id: 2, name: 'WideEye', description: 'Wideeye Potion' }, { id: 100, name: 'LiquidLuck', description: 'Felix Felicis' }, { id: 10, name: 'PolyJuice', description: 'Polyjuice Potion' }],
   update: function update() {
      //document.querySelector('#data').innerHTML = JSON.stringify(this.collect(), null, '\t')
   },
   match: function match(word, option) {
      return option.name.startsWith(word) || option.description.toLowerCase().indexOf(word.toLowerCase()) >= 0;
   },
   template: function template(option) {
